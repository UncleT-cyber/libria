...
export default function QueuePanel({ onClose, expanded, onToggleExpand }: Props) {
  ...
  return (
    <aside className={`${expanded ? 'w-[420px]' : 'w-[350px]'} flex-shrink-0 flex flex-col bg-[#121212] rounded-lg overflow-hidden transition-all duration-200`}>
      <div className="px-4 pt-4 flex items-center justify-between border-b border-[#232323]">
        <div className="flex items-center gap-6">
          <button
            onClick={() => setActiveTab('queue')}
            className={`pb-3 text-[16px] font-bold border-b-2 transition-colors ${activeTab === 'queue' ? 'text-white border-[#1DB954]' : 'text-[#b3b3b3] border-transparent hover:text-white'}`}
          >
            Queue
          </button>
          <button
            onClick={() => setActiveTab('recent')}
            className={`pb-3 text-[16px] font-bold border-b-2 transition-colors ${activeTab === 'recent' ? 'text-white border-[#1DB954]' : 'text-[#b3b3b3] border-transparent hover:text-white'}`}
          >
            Recently played
          </button>
        </div>
        ...
        {activeTab === 'queue' ? (
            ...
            <div className="px-2">
              ...
              {queue.slice(0, 10).map((t) => (
                <button key={t.id} onClick={() => playTrack(t.id)} className="w-full flex items-center gap-3 p-2 hover:bg-[#1a1a1a] rounded-md text-left group">
                  ...
                  <span className="text-[#b3b3b3] opacity-0 group-hover:opacity-100">•••</span>
                </button>
              ))}
            </div>
          </>
          ...
          <div className="px-2">
            ...
            {recentlyPlayed.length === 0 ? (
              <p className="text-sm text-[#6a6a6a] px-2">No recently played tracks yet. Play something to see it here.</p>
            ) : (
